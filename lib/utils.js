...
export default (config) => {
  ...
  const scanLocales = () => {
    ...
    const paths = globbySync([dirs, `!${files}`], {
      onlyFiles: false
    })
   ...
   */
  const compileHtmlBody = async (template, data, locale, returnMeta = false) => {
    ...
    // @note explicitly disabling minification (https://github.com/mjmlio/mjml/issues/2589)
    const { html } = await mjml2html(htmlBody, {
